...
public class CeoAccountApprovalController {
    ...
    public CeoAccountApprovalController(AccountProvisioningService service,
    ...
    List<SystemAdminAccountController.AccountResponse> pending(@AuthenticationPrincipal Jwt jwt) {
        return service.pendingCeoApproval(actorId(jwt)).stream()
    ...
    SystemAdminAccountController.AccountResponse approve(@PathVariable UUID id,
                                                          ...
                                                          @Valid @RequestBody SystemAdminAccountController.HrApprovalRequest request) {
        return SystemAdminAccountController.AccountResponse.from(
                hrOnboarding.approveByCeo(actorId(jwt), id, request.command()));
    }
    ...
    SystemAdminAccountController.AccountResponse reject(
        ...
        return SystemAdminAccountController.AccountResponse.from(
                service.rejectByCeo(actorId(jwt), id, request.reason()));
    }
