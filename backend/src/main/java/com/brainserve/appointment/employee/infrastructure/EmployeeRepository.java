...
public interface EmployeeRepository extends JpaRepository<Employee, UUID> {
    ...
    @Query("""
            select employee
             ...
             where (:departmentId is null or employee.departmentId = :departmentId)
               and (:status is null or employee.status = :status)
               and (:query is null
                    or lower(employee.displayName) like lower(concat('%', :query, '%'))
                    or lower(employee.employeeNumber) like lower(concat(:query, '%'))
                    or lower(employee.officialEmail) like lower(concat(:query, '%')))
            """)
    ...
    List<Employee> findAllByStatusOrderByDisplayNameAsc(EmployeeStatus status);
    ...
    @Query(value = """
            select department_id as "departmentId",
                   ...
                   count(*) filter (where status = 'ON_LEAVE') as "onLeaveEmployees",
                   count(*) filter (where status = 'ONBOARDING') as "onboardingEmployees"
              from employee
