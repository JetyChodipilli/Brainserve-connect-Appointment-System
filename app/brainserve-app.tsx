...
function EmployeesView({
  ...
  const [totalElements, setTotalElements] = useState(employees.length);
...
function AccountProvisioningPanel({ role, departments: initialApprovalDepartments, onDecision, compact = false }: {
  ...
  const createPrivileged = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setError(""); setMessage(""); setPreviewTemporaryPassword("");
    const form = event.currentTarget;
    const data = new FormData(form);
    try {
      const fullName = String(data.get("fullName")).trim();
      ...
      let created: ProvisioningAccount;
      if (isBackendConfigured) {
        created = await brainServeApi.createPrivilegedAccount(fullName, email, accountRole);
      } else {
        if (!["ROLE_CEO", "ROLE_HR_ADMIN", "ROLE_MANAGER"].includes(accountRole)) {
          throw new Error("Only CEO, HR Admin or Manager accounts can be created here.");
        }
        ...
        const governingCeo = existingAccounts.find((account) => account.role === "ROLE_CEO"
          && ["ACTIVE", "PENDING_APPROVAL"].includes(account.status));
        if (accountRole === "ROLE_CEO" && governingCeo) {
          throw new Error(`BrainServe Connect already has one governing CEO: ${governingCeo.fullName}.`);
        }
        if (accountRole !== "ROLE_CEO"
            && !existingAccounts.some((account) => account.role === "ROLE_CEO" && account.status === "ACTIVE")) {
          throw new Error("Activate the company CEO before creating an HR Admin or Manager account.");
        }
        const temporaryPassword = newDemoTemporaryPassword();
        const previewAccount: DemoProvisioningAccount = {
          id: newClientId(), fullName, email, role: accountRole, status: "PENDING_APPROVAL",
          createdByUserId: DEMO_SYSTEM_ADMIN.id, approvedByUserId: null,
          createdAt: new Date().toISOString(), approvedAt: null,
          forcePasswordChange: true,
          passwordHash: await hashDemoPassword(temporaryPassword),
        };
        writeDemoAccounts([...existingAccounts, previewAccount]);
        created = previewAccount;
      ...
      setMessage(accountRole === "ROLE_CEO"
        ? isBackendConfigured
          ? "The one CEO slot is reserved. The temporary password was emailed; System Admin approval is still required."
          : "The one CEO slot is reserved in this browser. Copy the temporary password, then approve the account."
        : isBackendConfigured
          ? "Account created in pending status and routed to the single company CEO for approval."
          : "Preview account created and routed to the single company CEO. Copy the temporary password before changing accounts.");
      form.reset();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The account could not be created."); }
  };
  ...
  return <section className="provisioning-section">
    ...
    {role === "System Admin" && <article className="panel glass-panel">
      ...
      <form className="staff-create-form" onSubmit={createPrivileged}>
        <label>Full name<input name="fullName" minLength={2} maxLength={170} required /></label>
        <label>Company email<input name="email" type="email" placeholder="name@brainserve.in" required /></label>
        <label>Role<select name="role" defaultValue={ceoSlotAvailable ? "ROLE_CEO" : "ROLE_HR_ADMIN"}>
          <option value="ROLE_CEO" disabled={!ceoSlotAvailable}>{ceoSlotAvailable ? "CEO" : "CEO · already assigned"}</option>
          <option value="ROLE_HR_ADMIN">HR Admin · CEO approval</option>
          <option value="ROLE_MANAGER">Manager · CEO approval</option>
        </select></label>
        <button className="button button-primary"><UserPlus size={16} /> Create pending account</button>
      </form>
