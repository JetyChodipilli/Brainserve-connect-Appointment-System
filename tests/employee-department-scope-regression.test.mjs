...
test("HR and Team Lead employee lists are derived from their assigned department", () => {
  ...
  assert.match(service, /departmentHrs\.requireForUser\(actorUserId\)\.departmentId\(\)/);
  assert.match(service, /teamLeads\.requireForUser\(actorUserId\)\.departmentId\(\)/);
...
test("a CEO department profile is visible to HR but protected from lifecycle actions", () => {
  assert.match(service, /requireLifecycleAuthority\(actorUserId, employee\)/);
  ...
  assert.match(service, /findByOfficialEmailIgnoreCase\(account\.email\(\)\)/);
  assert.match(service, /CEO_LIFECYCLE_PROTECTED/);
  assert.match(service, /rejectedSecurityAudit\.record\("CEO_LIFECYCLE_CHANGE_BLOCKED"/);
