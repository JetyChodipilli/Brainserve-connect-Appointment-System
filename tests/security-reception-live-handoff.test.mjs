...
test("appointment loading follows every page so new Security walk-ins reach Reception", () => {
  assert.match(api, /allSpringPageContent<ManagedAppointment>\("\/appointments\?sort=slotStart,asc"\)/);
  assert.match(api, /page=\$\{page\}&size=\$\{pageSize\}/);
  assert.match(api, /cache:\s*"no-store"/);
  assert.doesNotMatch(api, /appointments\?size=100/);
});

test("Reception accepts only today's security queue while backend persists the handoff status", () => {
  assert.match(app, /item\.status === "Awaiting Reception"/);
  assert.match(app, /officeToday\(new Date\(item\.slotStart\)\) === officeToday\(\)/);
  assert.match(service, /appointment\.recordSecurityIntake\(securityUserId/);
