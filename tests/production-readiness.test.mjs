...
const application = read("backend/src/main/java/com/brainserve/appointment/BrainServeAppointmentApplication.java");
const asyncConfig = read("backend/src/main/java/com/brainserve/appointment/notification/application/NotificationAsyncConfiguration.java");
const listener = read("backend/src/main/java/com/brainserve/appointment/notification/application/SecurityArrivalNotificationListener.java");
...
const eventContract = read("backend/src/main/java/com/brainserve/appointment/appointment/api/AppointmentEvents.java");
const leaveController = read("backend/src/main/java/com/brainserve/appointment/employee/api/LeaveRequestController.java");
const indexes = read("backend/src/main/resources/db/migration/V15__production_operational_indexes.sql");
...
test("production configuration never commits the bootstrap admin password", () => {
  ...
  assert.match(envExample, /^SYSTEM_ADMIN_DEFAULT_PASSWORD=\s*$/m);
  assert.equal(envExample.includes("SystemAdmin@BrainServe#06"), false);
});
...
test("locked accounts cannot mint access tokens through refresh", () => {
  assert.ok(authentication.includes(".filter(account -> !account.isLocked())"));
});

test("approved Employee accounts cannot authenticate before HR assigns an employee profile", () => {
  assert.ok(authentication.includes("EMPLOYEE_PROFILE_ASSIGNMENT_REQUIRED"));
  assert.ok(authentication.includes("SystemRole.ROLE_EMPLOYEE"));
  assert.ok(authentication.includes("user.getEmployeeId() == null"));
  assert.ok(authentication.match(/requireOperationalProfile\(user\)/g)?.length >= 2,
...
test("Reception forwarding carries and targets the selected host", () => {
  ...
  assert.ok(notifications.includes("activeByEmployeeId(hostEmployeeId)"));
});

test("special visits cannot check in until Reception forwards them", () => {
  assert.ok(appointments.includes("requiresCabinForward"));
  assert.ok(appointments.includes("appointment.getReceptionForwardedAt() != null"));
});

test("invalid leave decisions produce a client-safe business error", () => {
  assert.ok(leaveController.includes('"INVALID_LEAVE_DECISION"'));
  assert.ok(leaveController.includes("HttpStatus.UNPROCESSABLE_ENTITY"));
});
